...
git clone git@github.com:Nyantahi/homebrew-chibby.git ../homebrew-chibby
...
3. Users can then install with:
   ```bash
   brew tap Nyantahi/chibby
   brew install --cask chibby      # GUI + CLI
   # or
   brew install Nyantahi/chibby/chibby-cli  # CLI only
   ```
...
update-homebrew:
  ...
  steps:
    - uses: actions/checkout@v4
      with:
        repository: Nyantahi/homebrew-chibby
...
```bash
brew tap Nyantahi/chibby
brew install Nyantahi/chibby/chibby-cli
```
