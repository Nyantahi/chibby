...
fn get_styles() -> clap::builder::Styles {
    clap::builder::Styles::styled()
        ...
        .placeholder(
            clap::builder::styling::AnsiColor::BrightBlack.on_default(),
        )
}
...
async fn main() {
    ...
    let result = match &cli_args.command {
        ...
        Some(Commands::Run { env, stage, project, skip_preflight, dry_run }) => {
            run_pipeline(&printer, env.as_deref(), stage, project.as_ref(), *skip_preflight, *dry_run).await
        }

        Some(Commands::Status { project }) => {
            show_status(&printer, project.as_ref()).await
        }

        Some(Commands::Cancel { project }) => {
            cancel_pipeline(&printer, project.as_ref()).await
        }

        Some(Commands::Projects(cmd)) => {
            handle_projects(&printer, cmd).await
        }

        Some(Commands::Pipeline(cmd)) => {
            handle_pipeline(&printer, cmd).await
        }

        Some(Commands::History { project, env, limit }) => {
        ...
        Some(Commands::Rollback { run_id }) => {
            rollback_run(&printer, run_id).await
        }

        Some(Commands::Secrets(cmd)) => {
            handle_secrets(&printer, cmd).await
        }

        Some(Commands::Env(cmd)) => {
            handle_env(&printer, cmd).await
        }

        Some(Commands::Version(cmd)) => {
            handle_version(&printer, cmd).await
        }

        Some(Commands::Artifact(cmd)) => {
            handle_artifact(&printer, cmd).await
        }

        Some(Commands::Scan(cmd)) => {
            handle_scan(&printer, cmd).await
        }

        Some(Commands::Init { path, ai }) => {
            init_project(&printer, path.as_ref(), *ai).await
        }
        ...
        Some(Commands::Updater(cmd)) => {
            handle_updater(&printer, cmd).await
        }

        Some(Commands::Logs { run_id, follow }) => {
            show_logs(&printer, run_id, *follow).await
        }
    ...
    match cmd {
        ...
        ProjectsCmd::Add { path, name } => {
            ...
            let display_name = name.as_deref()
                .unwrap_or_else(|| path.file_name().and_then(|n| n.to_str()).unwrap_or("project"));
            
            printer.success(&format!("Added {}", display_name));
            printer.info(&format!("Run {} to generate pipeline", "chibby pipeline generate".cyan()));
        }
...
async fn handle_pipeline(printer: &Printer, cmd: &PipelineCmd) -> anyhow::Result<()> {
    match cmd {
        PipelineCmd::Show { project: _ } => {
            ...
            let stages = [
                ("preflight", "Preflight", "chibby scan secrets && chibby scan deps"),
                ("build", "Build", "npm run build"),
                ("test", "Test", "npm test"),
                ("deploy", "Deploy", "ssh deploy@server 'docker compose up -d'"),
            ];
...
async fn show_history(
    ...
    let runs = [
        ("abc123", StageStatus::Success, "2m ago", 34_000, Some("production")),
        ("def456", StageStatus::Success, "1h ago", 45_000, Some("staging")),
        ("ghi789", StageStatus::Failed, "2h ago", 12_000, Some("production")),
        ("jkl012", StageStatus::Cancelled, "3h ago", 8_000, None),
        ("mno345", StageStatus::Success, "1d ago", 67_000, Some("staging")),
    ];
...
async fn retry_run(
    ...
    // Would call run_pipeline with retry context
    Ok(())
}
...
async fn rollback_run(printer: &Printer, run_id: &str) -> anyhow::Result<()> {
    ...
    printer.warn("Rolling back to previous deployment...");
    
    // Would call run_pipeline with rollback context
    Ok(())
}
...
async fn handle_secrets(printer: &Printer, cmd: &SecretsCmd) -> anyhow::Result<()> {
    match cmd {
        ...
        SecretsCmd::Set { key, value: _, project: _ } => {
...
async fn handle_env(printer: &Printer, cmd: &EnvCmd) -> anyhow::Result<()> {
    match cmd {
        EnvCmd::List { project: _ } => {
            ...
            println!("  {} {}", icons::SUCCESS.green(), "production".white().bold());
...
async fn handle_version(printer: &Printer, cmd: &VersionCmd) -> anyhow::Result<()> {
    match cmd {
        ...
        VersionCmd::Bump { level, project: _, tag, changelog } => {
...
async fn handle_scan(printer: &Printer, cmd: &ScanCmd) -> anyhow::Result<()> {
    match cmd {
        ScanCmd::Secrets { project: _, baseline } => {
        ...
        ScanCmd::Deps { project: _, severity } => {
...
async fn init_project(printer: &Printer, path: Option<&PathBuf>, ai: bool) -> anyhow::Result<()> {
    ...
    let path = path.cloned().unwrap_or_else(|| std::env::current_dir().unwrap());
...
async fn handle_updater(printer: &Printer, cmd: &UpdaterCmd) -> anyhow::Result<()> {
    match cmd {
        ...
        UpdaterCmd::Publish { project: _, dry_run } => {
            if *dry_run {
